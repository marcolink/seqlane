...
contract. Plan snapshots preserve model selections under isolated and branch
session policies; reuse policies remain model-free. Public event types are
derived from the package-owned canonical Zod schema so the runtime contract
has one authoritative definition. It has no dependency on Mastra, executors,
Studio, terminal output, OpenTelemetry, or CloudEvents.
