...
const reviewFindingSchema = z.strictObject({
  ...
  effectiveSeverity: z
    ...
    .default("nit"),
  disposition: z
    .enum(["open", "fixed", "wont-fix", "downgraded", "not-reproducible"])
    .default("open"),
  status: z
    .enum(["new", "open", "addressed", "resolved", "reopened", "dismissed"])
    .default("open"),
...
const PUBLICATION_TRUNCATION_NOTICE =
  "⚠️ Publication text was truncated to fit the GitHub comment limit.";
...
export function renderPublication(
  ...
  const runs = priorLedger.runs.some(
    (run) => run.githubRunId === githubRunId && run.attempt === attempt,
  )
    ? priorLedger.runs
    : [...priorLedger.runs, current];
  ...
  const findings = [...report.findings].sort(
    (left, right) =>
      ...
      severityRank(left.effectiveSeverity ?? left.severity) -
        severityRank(right.effectiveSeverity ?? right.severity) ||
      left.id.localeCompare(right.id),
  );
  const blockers = findings.filter(
    (finding) =>
      active(finding.status) &&
      ["critical", "required"].includes(
        finding.effectiveSeverity ?? finding.severity,
      ),
  ).length;
  const advisories = findings.filter(
    (finding) =>
      active(finding.status) &&
      (finding.effectiveSeverity ?? finding.severity) === "optional",
  ).length;
      ...
      : [
          ...
          ...visible.map(
            (finding) =>
              ...
              mark +
              " | " +
              safeText(finding.effectiveSeverity ?? finding.severity) +
              " | " +
  ...
  const body = [
    ...
    ...(findings.length > visible.length
      ? [
          ...
          "⚠️ Showing " +
            ...
            " of " +
            findings.length +
            " retained findings. The machine-readable state contains the complete bounded set.",
        ]
      : []),
    ...(report.limitations.length === 0
      ...
      : [
          "",
          "## Review limitations",
          ...report.limitations.map((value) => "- ⚠️ " + safeText(value)),
        ]),
    "",
    "<details>",
    "<summary>Run metrics (" +
      ...
      totalCost.toFixed(6) +
      " · last run: $" +
      (runs.at(-1)?.metrics.totalCost ?? 0).toFixed(6) +
      "</summary>",
    "",
  ...
  const boundedBody =
    body.length <= MAX_PUBLICATION_BODY_CHARS
      ...
      : body.slice(
          0,
          MAX_PUBLICATION_BODY_CHARS - PUBLICATION_TRUNCATION_NOTICE.length - 2,
        ) +
        "\n\n" +
