...
const TRUSTED_REPORT_AUTHORS = new Set(["github-actions", "github-actions[bot]"]);
...
function normalizeComment(value: object, kind: "issue" | "review"): ReviewComment {
  ...
  const allCommands = lines.flatMap((line) => {
    ...
    let effectiveSeverity: "critical" | "required" | "optional" | "nit" | undefined;
    if (action === "downgrade") {
      const downgradeSeverity = match[3]?.trim().match(/^(?:to\s+)?(critical|required|optional|nit)(?:\s+(?:reason\s*[:=]\s*)?(.*))?$/i)?.[1];
      if (downgradeSeverity === undefined) return [];
      effectiveSeverity = downgradeSeverity.toLowerCase() as typeof effectiveSeverity;
    }
...
function githubError(code: string, message: string, cause?: unknown): CodeReviewError {
...
export class GitHubReviewAdapter implements GitHubReviewPort {
  ...
  async readPullRequest(number: number): Promise<PullRequestContext> {
    try {
      ...
      if (!parsed.success) throw githubError("MALFORMED_PULL_REQUEST", "GitHub returned malformed pull-request data.", parsed.error);
      return parsed.data;
    } catch (cause) {
      if (cause instanceof CodeReviewError) throw cause;
      throw githubError("PULL_REQUEST_READ_FAILED", "Could not read pull-request data.", cause);
    }
  }
  ...
  async readComments(number: number): Promise<ReviewHistory> {
    try {
      ...
      if (!Array.isArray(issues) || !Array.isArray(reviews)) {
        throw githubError("MALFORMED_COMMENTS", "GitHub returned malformed comment data.");
      }
      const values = [
        ...issues.map((value) => [value, "issue"] as const),
        ...reviews.map((value) => [value, "review"] as const),
      ].filter((entry): entry is readonly [object, "issue" | "review"] => typeof entry[0] === "object" && entry[0] !== null)
      ...
      for (const comment of values) {
        ...
        if (previous === undefined || (previous.updatedAt ?? previous.createdAt) <= (comment.updatedAt ?? comment.createdAt)) latestByIdentity.set(key, comment);
      }
      const ordered = [...latestByIdentity.values()].sort(
        (left, right) =>
      ...
      throw githubError("COMMENTS_READ_FAILED", "Could not read review comments.", cause);
    }
  }

  async readAuthoritativeReport(number: number): Promise<ReviewComment | undefined> {
    const { comments } = await this.readComments(number);
    return [...comments].reverse().find((comment) =>
      (comment.author === "github-actions" || comment.author === "github-actions[bot]") &&
  ...
  async readIssueComment(commentId: string): Promise<ReviewComment> {
    try {
      ...
      if (typeof value !== "object" || value === null) {
        throw githubError("MALFORMED_COMMENT", "GitHub returned a malformed comment.");
      }
      return normalizeComment(value, "issue");
    } catch (cause) {
      if (cause instanceof CodeReviewError) throw cause;
      throw githubError("COMMENT_READ_FAILED", "Could not read the review marker.", cause);
    }
  }

  async createReport(number: number, body: string): Promise<void> {
    try { await this.client.createIssueComment(number, body); }
    catch (cause) { throw githubError("REPORT_CREATE_FAILED", "Could not publish the review report.", cause); }
  }
  async updateReport(commentId: string, body: string): Promise<void> {
    try { await this.client.updateIssueComment(commentId, body); }
    catch (cause) { throw githubError("REPORT_UPDATE_FAILED", "Could not update the review report.", cause); }
  }
  async deleteComment(commentId: string): Promise<void> {
    try { await this.client.deleteIssueComment(commentId); }
    catch (cause) { throw githubError("MARKER_DELETE_FAILED", "Could not remove the review marker.", cause); }
  }
}
