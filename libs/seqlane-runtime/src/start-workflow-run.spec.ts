...
const inputSchema = { parse: (value: unknown) => {
  if (typeof value !== "object" || value === null || !("value" in value)) throw new TypeError("value is required");
...
const workflow = createFlow({ id: "direct-runtime-test", input: inputSchema, output: outputSchema })
...
describe("startWorkflowRun", () => {
  ...
  it("supports caller identities and cancellation before task work", async () => {
    let executed = false;
    const cancellationWorkflow = createFlow({ id: "cancel", input: inputSchema, output: outputSchema })
      .task("local", defineTask({ ...localTask, execute: async (value: { value: string }) => { executed = true; return value; } }), ({ input }) => ({ value: input.value }))
      .output(({ tasks }) => tasks.local.output)
      .define();
    const { sink: eventSink, events } = createSink();
    const handle = startWorkflowRun({
    ...
    expect(events).toEqual([
      expect.objectContaining({ type: "run.started", workId: "work-fixed", runId: "run-fixed" }),
      expect.objectContaining({ type: "run.cancelled", workId: "work-fixed", runId: "run-fixed" }),
    ]);
  });
});
