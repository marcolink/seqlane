...
export class EffectSubprocessInterruptedError extends Error {
  constructor(override readonly cause?: unknown) {
    super("Effect subprocess was interrupted", { cause });
...
function interruption(signal: AbortSignal | undefined) {
  ...
  if (signal.aborted) {
    return Effect.fail(new EffectSubprocessInterruptedError(signal.reason));
  }

  return Effect.async<never, EffectSubprocessInterruptedError>((resume) => {
    const abort = () =>
      resume(Effect.fail(new EffectSubprocessInterruptedError(signal.reason)));
    signal.addEventListener("abort", abort, { once: true });
    return Effect.sync(() => signal.removeEventListener("abort", abort));
  });
}

export function effectSubprocess(request: EffectSubprocessRequest) {
...
export function runEffectSubprocess(
  request: EffectSubprocessRequest,
): Promise<EffectSubprocessResult> {
  return Effect.runPromiseExit(
    Effect.provide(effectSubprocess(request), nodeContextLayer),
  ).then((exit) => {
    if (Exit.isSuccess(exit)) return exit.value;
    const failure = Cause.failureOption(exit.cause);
    return Promise.reject(Option.isSome(failure) ? failure.value : exit.cause);
  });
}
