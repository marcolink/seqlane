# TS-013 — Local Read-Only Execution Studio

**Status:** Implemented
**Implements:** ADR-013
**Depends:** TS-002, TS-007, TS-011
**Scope:** MVP local browser inspection for concurrent Seqlane runs
...
- [ADR-013 — Provide a Local Read-Only Execution Studio](ADR-013-local-read-only-execution-studio.md) defines the local read-only Studio decision.
- [ADR-002 — Dedicated Runner Process](ADR-002-dedicated-runner-process.md) defines the runner and CLI ownership boundary.
- [ADR-007 — Work, Run, and Invocation Identity](ADR-007-work-run-invocation-identity-model.md) defines Seqlane execution identity.
- [ADR-008 — Executor-Neutral Workflow Authoring](ADR-008-executor-neutral-workflow-authoring.md) defines the executor-neutral boundary.
- [ADR-011 — Execution Output Package](ADR-011-dedicated-seqlane-output-package.md) defines event projection as a consumer concern.
- [RFC-002 — Execution Observability and Debugging](RFC-002-execution-observability-and-debugging.md) defines structured execution inspection and redaction principles.
