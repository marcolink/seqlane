# TS-002-08 — Run a selected workflow through the installed and development CLI
...
- Keep workflow discovery out of this command's first implementation. Until ADR-006 is implemented, accept an explicit, resolvable workflow reference and convert it to the existing `WorkflowReference`; do not silently invent name-resolution rules.
...
## Out of Scope
- Repository/user workflow discovery, `list`, and `plan` commands; ADR-006 owns discovery and ambiguity rules.
