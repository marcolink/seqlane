# RFC 1 — Seqlane Technical Architecture

**Status:** Draft
**Depends on:** Seqlane PRD
**Related:** RFC 2 — Execution Observability and Debugging
**Initial implementation profile:** Seqlane MVP
...
RFC 2 defines observability behavior.

RFC 1 requires the runtime to preserve enough structure to support it. At minimum, runtime events must retain Work ID when available, Run ID, Invocation ID, Task ID, workflow identity, dependency relationships, executor identity, status transitions, validation result, session metadata where relevant, errors, and timestamps.
