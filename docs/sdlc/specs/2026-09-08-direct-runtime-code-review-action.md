...
- Ship the code-review Action as a self-contained committed bundle.
...
must not use a review-target path as executable module input. It must not run
`pnpm install`, a Seqlane build, or a CLI build in the consumer workflow.

The main and post bundles must be self-contained and committed. Bundle builds
and drift checks must prove that `dist/main.js` and `dist/post.js` match
trusted Action source and its declared dependencies.
...
Retain the trusted source checkout because it supplies the local Action and
its committed bundle. Retain the untrusted review-target checkout and all
service Action composition. Keep the current workflow admission, permissions,
concurrency, timeout, and closed-pull-request cancellation behavior.
...
- Run runtime and Action typecheck, build, lint, bundle build, and bundle drift.
...
- `startRun` delegates shared execution to the service and retains fresh-runner
  behavior.
- `actions/code-review` runs from a committed self-contained Node 24 bundle.
