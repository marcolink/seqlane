---
id: spec.work-run-invocation-identity-model
title: Work, Run, and Invocation Identity Model
status: active
owners:
  - core
created: 2026-09-02
updated: 2026-09-03
upstream:
  - adr.work-run-invocation-identity-model
supersedes: []
---

# Work, Run, and Invocation Identity Model

> Migrated from legacy technical specification `TS-007`.
...
spec.seqlane-plan-ir-typed-dataflow currently uses `invocationId` for deterministic Plan construction and binding resolution. That value is a static address such as `task:1`; it repeats when the Plan is built in another Run and therefore cannot satisfy adr.work-run-invocation-identity-model's execution-identity definition.
...
`PlanNodeId` remains deterministic per Plan build as spec.seqlane-plan-ir-typed-dataflow requires. `InvocationId` is allocated only for execution; it must not be derived from, serialized with, or stored back into the Plan.
...
spec.work-run-invocation-identity-model is complete when:
...
- [adr.work-run-invocation-identity-model — Distinguish Work, Run, and Invocation Identity](../adrs/2026-09-02-work-run-invocation-identity-model.md) defines the canonical hierarchy and MVP exclusions.
- [rfc.seqlane-technical-architecture — Seqlane Technical Architecture](../rfcs/2026-09-02-seqlane-technical-architecture.md) assigns execution identities and structured events to Seqlane-owned contracts.
- [MVP — Seqlane](../prd/2026-09-02-seqlane.md) defines one fresh runner process per execution and excludes persistent Work, cross-run continuation, and Git provenance.
- spec.mastra-runtime-integration, spec.dedicated-runner-process, spec.seqlane-plan-ir-typed-dataflow, and spec.executor-neutral-workflow-authoring define the current runtime, runner, Plan, and executor-neutral boundaries this migration updates.

## Traceability

- [adr.work-run-invocation-identity-model](../adrs/2026-09-02-work-run-invocation-identity-model.md)
