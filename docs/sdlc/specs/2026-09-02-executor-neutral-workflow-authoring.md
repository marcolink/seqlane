...
# Executor-Neutral Workflow Authoring

> Migrated from legacy technical specification `TS-008`.
...
The runtime resolves bindings and validates input before it calls the resolved implementation. It validates output after the call as spec.mastra-runtime-integration defines.
...
Do not rewrite accepted ADRs. adr.opencode-executor-integration remains the private adapter decision. adr.executor-neutral-workflow-authoring supersedes its workflow-authoring portion.
...
spec.executor-neutral-workflow-authoring is complete when:
...
spec.executor-neutral-workflow-authoring does not implement:
