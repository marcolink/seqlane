...
- The package depends on @seqlane/core for canonical serialized event
  contracts and shared Seqlane primitives.
...
spec.local-read-only-execution-studio adds separate, optional `invocation.input` and `invocation.result`
...
Rollback or compensation execution is not introduced by spec.seqlane-execution-output-package. If a future
...
spec.seqlane-execution-output-package does not implement:
...
spec.seqlane-execution-output-package is complete when:
...
- [adr.dedicated-seqlane-output-package](../adrs/2026-09-02-dedicated-seqlane-output-package.md)
