...
> Superseded by [prd.seqlane-on-mastra](./2026-09-03-seqlane-on-mastra.md).
...
### R03: Extend existing automation
...
### R06: Preserve repository agent capabilities
...
### R11: Existing OpenCode environment
...
The MVP proves that an engineer can run one autonomous, typed workflow from the
CLI against an existing repository and receive a clear result. Technical engine,
process, protocol, and package contracts belong to the linked RFCs and specs.

### Reference use case

The reference workflow remediates a failed Renovate dependency update by
investigating the failure, planning and applying a repair, verifying the result,
and returning a structured outcome without user interaction after startup.

### Included outcomes

- Discover and run repository and user workflows from the CLI.
- Compose reusable tasks and workflows with validated inputs and outputs.
- Use the repository agent environment and an available OpenCode runtime.
- Observe progress, final success or failure, and cancellation.
- Reject work that requires interactive approval or clarification.

### Excluded outcomes

The MVP does not require a graphical interface, interactive or durable workflows,
human approval steps, scheduling, parallel execution, automatic retry or repair
loops, multiple coding-agent backends, managed OpenCode lifecycle, persistent
history, or cross-run continuation.

### Milestone success

The milestone succeeds when a real failed Renovate update can be investigated,
repaired, verified, and reported end to end through Seqlane with no user decision
after execution starts.

## Traceability

- [brd.seqlane: Seqlane](../brd/2026-09-02-seqlane.md)
- Replaced by [prd.seqlane-on-mastra: Seqlane on Mastra](./2026-09-03-seqlane-on-mastra.md).
