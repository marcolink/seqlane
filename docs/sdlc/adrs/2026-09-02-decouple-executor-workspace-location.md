...
# Decouple Executor Workspace Location from Seqlane Execution Location
...
neutral under adr.executor-neutral-workflow-authoring.
...
adr.opencode-executor-integration. Rejected for the current runtime model; managed per-Run servers remain
...
- Document that external-server mode does not start or stop the server, as
  established by adr.opencode-executor-integration.
