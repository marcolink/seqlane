# ADR-014 — Simplify the Local Development Studio Trust and Lifecycle

**Status:** Implemented
**Scope:** Local Studio access, service discovery, and process lifecycle
**Related:** ADR-002, ADR-013, TS-013
**Supersedes:** The access-boundary, session-discovery, and lifecycle details in ADR-013

## Context

ADR-013 defines a loopback-only, read-only Studio service with per-session
...
policy, loopback restriction, and executor-neutral contracts from ADR-013
remain in force.
...
- Remove descriptor and bootstrap-token assumptions from TS-013 and the
