...
// @test-scope ../../libs/read-context/src/index.ts
...
describe("retrieval workflow fan-out", () => {
  it("runs independent evidence scrapes before selection", () => {
    ...
    expect(retrieval?.plan.nodes.at(-1)).toMatchObject({
      type: "task",
      taskId: "read-context-select-evidence",
      dependsOn: [
        "read-context-exact-search:1",
        "read-context-zvec-search:1",
        "read-context-ripwire-search:1",
      ],
    });
  });
});
